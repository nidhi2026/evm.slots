...
export const NetworksDropdown = ({ onChange }: { onChange: (options: any) => any }) => {
  ...
  const handleSelectChange = (newValue: SingleValue<Options> | MultiValue<Options>) => {
    ...
    if (selected?.value === "other-chains") {
      ...
      if (selected) {
        const chain = (Object.values(chains) as Chain[]).find(chain => chain.id === selected.value);
  ...
  return (
    <>
      <Select
        ...
        components={{
          Option: props => <CustomOption {...props} onDelete={handleDeleteCustomChain} />,
        }}
        isSearchable={!isMobile}
        className="max-w-xs relative text-sm w-44 text-left"
        theme={theme => ({
          ...theme,
          colors: {
            ...theme.colors,
            primary25: isDarkMode ? "#33476fff" : "#dae8ff",
            primary50: isDarkMode ? "#323f61" : "#dae8ff",
            primary: isDarkMode ? "#6d90d2ff" : "#709be0ff",
            neutral0: isDarkMode ? "#2a3656" : theme.colors.neutral0,
            neutral80: isDarkMode ? "#ffffffff" : theme.colors.neutral80,
          },
        })}
        styles={{
          menuList: (provided: any) => ({ ...provided, maxHeight: 280, overflow: "auto" }),
